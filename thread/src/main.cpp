...
mutex mutexQueue; // déclaration du mutex pour protéger l'accès aux données de la voie SPI
queue<char *> tramesRecues;
...
void traitementTrame()
{
    while (true)
    {
        lock_guard<mutex> lock(mutexQueue); // verrouillage du mutex
        if (!tramesRecues.empty())
        {
            cout << "traitement de la dernière trame reçue " << endl;
            cout << tramesRecues.front();
            tramesRecues.pop(); 
        }
    }
}
